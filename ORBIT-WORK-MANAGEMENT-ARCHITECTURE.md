...
#### Project roles are tenant-defined, not a fixed enum (implemented v1.44)

"Project role membership" in the diagram above resolves through a tenant-scoped `Role`/`RolePermission`
schema (`Domain.Access.Role`), not a fixed `Administrator`/`Member`/`Viewer` enum. A `Role` is a named,
per-tenant row carrying a `RolePermission` set drawn from the fixed `ProjectPermission` catalog (`View`,
`CreateWorkItem`, `TransitionWorkItem`, `Administer` — the catalog itself stays a compile-time enum, per
ADR-003's "permission is a query predicate" of fixed capabilities). Every tenant is seeded with three
`IsSystem` roles reproducing the old enum's exact grants; tenant admins (`ITenantAuthorization.CanManageRoles`,
same Owner/Administrator gate as `CanManageTeams`) can edit any role's permission set — including system
roles — or create additional custom roles, and `ProjectRoleAssignment`/`ProjectGroupRoleAssignment` now
carry a `RoleId` instead of an enum value. `ProjectAccessQuery` resolves permission by joining the
assignment to `RolePermission` instead of the old hardcoded `ProjectPermissionRoles` switch. `TenantRole`
(workspace-level Owner/Administrator/Member) intentionally stays an enum for now — it carries singleton
owner-lifecycle semantics (advisory-lock-serialized, §13.5) that a generic role/permission schema doesn't
yet model, so tenant-level custom roles remain a distinct, not-yet-started increment (§13.5, §10.4).

```mermaid
erDiagram
    ROLE ||--o{ ROLE_PERMISSION : grants
    ROLE ||--o{ PROJECT_ROLE_ASSIGNMENT : "assigned via"
    ROLE ||--o{ PROJECT_GROUP_ROLE_ASSIGNMENT : "assigned via"
    ROLE {
        guid id
        guid tenant_id
        string name
...
| Access control | Owner/admin/member tenant roles; a `MembershipTier` (Standard/Guest) orthogonal to `TenantRole`, enforced at domain validation, a Postgres check constraint, and the project-access query's tenant-wide-access shortcut so a guest only sees explicitly assigned projects (v1.24); custom, fully user-defined **project** roles and permission sets — a tenant-scoped `Role`/`RolePermission` schema (v1.44) that replaced the `ProjectRole` enum and its `ProjectPermissionRoles` switch, with an admin-facing Settings UI to create roles and edit their permission grants; directory-group role assignments carry the same `RoleId`; shared permission predicates inside project/work-item queries; existence-hiding reads; authorization epoch/cache; advisory-lock-serialized owner lifecycle | Custom, fully user-defined **tenant/workspace**-level roles replacing the `TenantRole` enum — deliberately not attempted alongside the v1.44 project-role schema because `TenantRole.Owner` carries singleton, advisory-lock-serialized ownership-lifecycle semantics a generic role/permission schema doesn't yet model; permission schemes and dynamic grants, issue security, role-management audit log |
...
| Agile UI | Persisted boards, configurable columns/WIP limits, fractional ranking, and Future/Active/Closing/Closed/Reopened sprint lifecycle; temporal memberships; atomic sprint completion, idempotent operation records, reopening, and sprint-scope facts with `EstimateDelta` written on scope/status/estimate changes; batched sprint/member reads; sprint burndown/scope-change/velocity report projected from the immutable fact log, reproducible for closed sprints (`GET /sprints/{id}/report`, v1.21); backlog/board search and an assignee quick-filter backed by an enriched member directory (v1.20); cumulative-flow, cycle-time, and control-chart reports folded from sprint-scope facts and the existing "Status" `WorkItemHistoryEntry` log (`GET /sprints/{id}/reports/{cumulative-flow,cycle-time,control-chart}`, v1.35); cross-column and same-column board drag (v1.11/v1.12), drag-and-drop backlog↔sprint and sprint↔sprint reassignment (v1.36), and assignee-grouped board swimlanes (S6.1.5, v1.36); project-owned `WorkItemStatusDefinition` workflow-status catalog replacing the fixed `WorkItemStatus` enum, with an "Edit workflow" add/rename/recolor/reorder/delete UI (v1.45); a live sprint-insights panel (progress, overdue/stuck/blocked/flagged items for attention, per-epic completion, `GET /sprints/{id}/insights`, v1.45); `Sprint.Edit` for renaming/redating a future or active sprint (`PATCH /sprints/{id}`, v1.45); per-user `BoardViewPreference` for hide-done-after, column size, and card-field visibility (v1.45); sprint completion's rollover choice is now a yes/no decision — auto-create and move incomplete items into the next sprint (name/dates derived from the closed sprint), or move them to the top of the backlog — replacing the v1.13-era "pick an existing Future sprint" dropdown (v1.47); the sprint-insights panel's burndown chart and scope-change counts now render from the same `GET /sprints/{id}/report` fact-log projection and the handler's `AddedAfterStartCount`/`RemovedAfterStartCount` instead of the v1.45 placeholder SVG path and hardcoded zero/one counts (v1.48); backlog work-item selection opens an inline, pointer-resizable split-view detail panel (`BacklogView`'s embedded `WorkItemDetailView`, width persisted client-side in `localStorage`) instead of only a drawer/modal; a backlog insights panel (planning-sprint commitment points, top-5 work-type breakdown) and a backlog view-settings panel (row density, empty-sprint visibility, epic/due-date/status/estimate column toggles) open from new toolbar buttons (v1.48); per-user `BacklogViewPreference` persists the backlog view-settings panel's row density, empty-sprint visibility, and field toggles server-side, mirroring `BoardViewPreference` (v1.49) | Parallel active sprints, worker-backed completion for very large sprints, virtualization, guarded workflow transitions (conditions/validators/post-functions per §4.2); backlog insights' "Recommended" velocity figure remains UI-only — no velocity-average computation yet, so it still resets per session and per device |
...
| Notifications | Event/channel/digest/quiet-hours/self-notify controls | Versioned preference API and UI implemented; every outbox email trigger (§10.5) now consults `EmailEnabled`/`SelfNotify` (v1.25-v1.30) and schedules delivery against `DigestCadence`/quiet hours via `OutboxEmailMessage.NotBefore` (v1.38); sprint-completed's recipient resolution changed from the sprint's work-item owner fields to every user holding `ProjectPermission.View` on the project — i.e. everyone with board/backlog access, not just item owners (v1.47); sprint-started is unchanged and still owner-field-based | Add preview/test delivery; digest cadence currently delays each event's own email to the next digest window rather than batching several events into one combined email body |
...
| Board settings | Kanban/Scrum type, filter, columns, estimation, WIP and sprint behavior | Persisted versioned board aggregate (name, type, `Version`) with an owned `board_columns` collection (status, order, nullable WIP limit, Warn/Block mode), edited together under one `If-Match` PATCH (v1.11/v1.12); columns now reference the project's editable `WorkItemStatusDefinition` catalog instead of a fixed enum (v1.45), reachable from the Board tab's `...` menu alongside the new "Edit workflow", "Sprint insights", "View settings", and "Edit sprint" panels | Add filter/estimation configuration, atomic publish and historical config retention for report reproducibility |
...
26. ~~On sprint close, ask whether incomplete items should move to a new sprint or the top of the backlog, and notify everyone with board/backlog access instead of just item owners.~~ **Done in v1.47:** `CompleteSprintCommand.RolloverTargetSprintId: Guid?` (pick an existing Future sprint) became `CreateRolloverSprint: bool`. When true, `CompleteSprintHandler` auto-creates the next sprint (`Sprint.NextRolloverName` increments the closed sprint's trailing number, e.g. "Sprint 5" → "Sprint 6", falling back to a date-based name; dates derived from the closed sprint's end date and duration) and moves every not-Done item into it. When false, those items are reordered to sit above every existing backlog item via a new `IWorkItemRepository.GetMinBacklogRankAsync`, instead of just falling off the sprint wherever their existing rank happened to place them. The idempotent-retry path (`SprintCompletionOperation`) now stores the handler-created sprint's id rather than a caller-supplied one, and a resumed close reuses it instead of creating a second sprint. Sprint-completed notifications switched from the sprint's work-item owner fields to every user with `ProjectPermission.View` on the project, via a new `IProjectAccessRepository.ListUserIdsWithPermissionAsync` (mirrors `ProjectAccessQuery.PermittedProjects`'s tenant-wide-shortcut/explicit-assignment/group-assignment policy, inverted from "which projects can this principal see" to "which users can see this project"); sprint-started notifications are unchanged. Frontend: `CompleteSprintDialog.tsx`'s existing-Future-sprint dropdown and `BacklogView.tsx`'s separate inline rollover picker were both replaced with a shared `RolloverChoice` yes/no control (`web/src/features/board/RolloverChoice.tsx`), previewing the auto-created sprint's name via a `nextRolloverName` TS port of the same naming rule (`web/src/lib/sprintNaming.ts`). Backend: 550 domain+application+architecture+integration tests green (2 new `ProjectAccessRepositoryTests`/`WorkItemBacklogRankTests` integration tests exercise the new EF queries against real Postgres, seeding tenant context directly via `TenantContext.SetTenant` rather than the HTTP header path other integration tests use, since these test a repository query in isolation). Frontend: `tsc -b`, `oxlint`, `vitest run` (97 tests) green; `npm run build` clean. Not attempted this pass: multi-instance rollover targeting (still exactly one auto-created sprint per close), and a live-browser click-through (verify visually before shipping).

27. ~~Persist the backlog view-settings panel's row density, empty-sprint visibility, and field toggles, closing the gap flagged since v1.48.~~ **Done in v1.49:** new `BacklogViewPreference` per-user/per-project aggregate, mirroring `BoardViewPreference`'s zero-version-sentinel GET / `If-Match` PATCH shape exactly. See the v1.49 revision-history entry below.
...
#### 13.5.6 v1.48 — Backlog insights, backlog view settings, inline backlog preview, and real sprint-insights data

This increment has two parts: finishing off placeholder UI that shipped with the v1.45 sprint-insights panel, and giving the backlog view the same "insights + view settings + inline preview" shape the board already had.

**Sprint-insights panel now renders real data.** `SprintInsightsPanel.tsx`'s burndown chart was a fixed hand-coded SVG path (`Aug 19`/`Sep 2` literal labels, a hardcoded 5-point remaining-work polygon) regardless of the sprint queried. It now fetches `GET /sprints/{id}/report` (the same v1.21 burndown projection the Sprint Report dialog already uses) and plots `burndown[].remainingPoints` against a straight-line guideline from `committedPoints` to zero, falling back to an explicit "sprint hasn't started" message when there's no baseline rather than drawing a chart that lies. The scope-change tiles' "Added"/"Removed"/"Modified" counts were likewise hardcoded (`↑ 0 work items`, `↓ 0 work items`, `● 1 work item`) regardless of what actually changed; `SprintInsightsDto` (`src/Orbit.Application/Boards/SprintInsights.cs`) gained `AddedAfterStartCount`/`RemovedAfterStartCount`, computed by the same distinct-`WorkItemId`-over-`SprintScopeFact` fold `SprintReportPoints.ComputeScopeChangeCounts` already used internally for a related figure, and the "Modified" tile was renamed "Net change" to describe what it actually shows (added count + removed count, not a third independent figure). The per-epic list's hardcoded "SCRUM-6 Test epic / 0% done" placeholder row is now a real "No epics in this sprint yet" empty state when `insights.epics` is empty.

**Backlog gets the board's insights/settings/inline-preview pattern.** Three additions to `BacklogView.tsx`, mirroring the board's existing `SprintInsightsPanel`/`BoardViewPreference` shape but scoped to the backlog and, unlike the board's version, not yet backed by persistence:

- `BacklogInsightsPanel.tsx` — a right-side panel (toolbar `LineChart` button) showing the nearest Active/Reopened/Future sprint's committed story points and a top-5 work-item-type breakdown (of that sprint's scope, or the whole backlog when there's no sprint to plan into). The "Recommended" commitment figure is an explicit "Not available yet" placeholder — velocity-average-from-closed-sprints is real future work, not a stub pretending to be a real number.
- `BacklogViewSettingsPanel.tsx` — a right-side panel (toolbar `SlidersHorizontal` button) toggling row density (Default/Compact), empty-sprint-section visibility, and per-column visibility for epic chip / due date / status / estimate. `BacklogView` reads these into `renderEpicChip`/`renderDueDate`/`renderEstimate` render-gates and a `rowPaddingClass`. State is local `useState` with no API call and no `localStorage` key, so it resets on reload — unlike the board's `BoardViewPreference`, which is a persisted per-user aggregate (§13.5 Agile UI row). Two rows ("Epic panel" toggle, "Work type"/"Work item key" field toggles) render as permanently-disabled switches, signaling planned-but-not-wired controls rather than omitting them.
- Inline split-view preview — clicking a backlog row now sets `selectedWorkItemId` and renders `WorkItemDetailView` directly inside `BacklogView` in a side panel (distinct from the pre-existing drawer/modal opened from other views), whose width is user-draggable via a `pointerdown`/`pointermove` handle and persisted to `localStorage['orbit_backlog_panel_width']` (clamped 380–900px) — the one piece of this increment's new UI state that *is* persisted, because it's a per-device layout preference rather than data. Covered by a new `BacklogView.test.tsx` (open-on-click, close-button-closes).

`WorkItemDetailView.tsx` was restructured (collapsible details section via `panelDetailsCollapsed`, reorganized breadcrumb/meta/panel markup) so the same component renders correctly at the narrower widths this increment introduces everywhere it's embedded: the backlog split panel above, and the existing modal/drawer, both shrunk from a near-full-viewport treatment (1440px modal, edge-to-edge full-height drawer) to a smaller floating dialog (1040px modal, rounded/inset drawer) in `App.css`.

**Status:** Frontend-only for the two new backlog panels and the layout/CSS changes (no new endpoints, no migration); `AddedAfterStartCount`/`RemovedAfterStartCount` are the only backend change, additive fields on an existing DTO. Not yet done: the insights panel's velocity recommendation (called out as a gap in the Agile UI row above), and an "Epic panel" toggle that has no corresponding epic panel to toggle yet. Backlog view-settings persistence, the other gap flagged here, closed in v1.49.
...
| 1.44 | 2026-08-21 | Custom project roles: `Role`/`RolePermission` schema (§4.3, §13.5 Access control) | Closes the "custom, fully user-defined roles and permission sets" gap flagged since v1.24, scoped to **project** roles only: `TenantRole` (workspace-level Owner/Administrator/Member) deliberately stays an enum this pass, since `TenantRole.Owner` carries singleton, advisory-lock-serialized ownership-lifecycle semantics a generic role/permission schema doesn't yet model — conflating that with a generic RBAC rewrite in one pass would have risked tenant-bootstrap invariants for no clear benefit. New `Domain.Access.Role` aggregate (tenant-scoped, named, `IsSystem` flag) backed by a `RolePermission` child collection drawn from the existing fixed `ProjectPermission` catalog (`View`/`CreateWorkItem`/`TransitionWorkItem`/`Administer` — the catalog itself is unchanged, per ADR-003's "permission is a query predicate" of fixed capabilities); `ProjectRoleAssignment`/`ProjectGroupRoleAssignment` now carry a `RoleId` instead of the old `ProjectRole` enum, which is removed entirely. `ProjectAccessQuery` resolves permission by joining the assignment to `RolePermission` (`dbContext.RolePermissions.Any(rp => rp.RoleId == assignment.RoleId && rp.Permission == permission)`) instead of the old hardcoded `ProjectPermissionRoles.For` switch, which is deleted. Every tenant (existing and new) is seeded with three `IsSystem` roles — Administrator (all four permissions), Member (View/CreateWorkItem/TransitionWorkItem), Viewer (View) — reproducing the old enum's exact grants, at every tenant-provisioning seam (`Bootstrap`, self-service `SignUp`, Google sign-in provisioning, `CreateWorkspace`, `CreateWorkspaceInOrganization`) via a shared `Role.SeedSystemRoles` domain helper, so no existing tenant's effective access changes. New `Application/Access/Roles.cs` (`ListRoles`/`CreateRole`/`RenameRole`/`UpdateRolePermissions`/`DeleteRole`) and `Api/Endpoints/RoleEndpoints.cs` (`GET/POST /roles`, `PATCH /roles/{id}`, `PUT /roles/{id}/permissions`, `DELETE /roles/{id}`), gated by a new `ITenantAuthorization.CanManageRoles()` (same Owner/Administrator tenant-role gate as `CanManageTeams`); `IsSystem` roles can have their permissions edited (the whole point of "fully user-defined permission sets") but not be renamed or deleted, and a role with active assignments can't be deleted. Migration `AddRoleAndRolePermission` creates `roles`/`role_permissions`, backfills every existing tenant's system roles and every existing `project_role_assignments`/`project_group_role_assignments` row's `role_id` via raw SQL keyed on tenant + old enum name, then drops the old `project_role` column and check constraints — live-verified against the local dev database (24 tenants seeded, 4 existing assignments backfilled with zero nulls). Frontend: `api/types.ts` gained `Role`/`ProjectPermission`, dropped the `ProjectRole` string union; Settings > Members & roles' per-project-role dropdown now renders live roles instead of a hardcoded three-option list, and a new "Project roles" panel lets a workspace admin create custom roles and toggle each role's permission checkboxes inline. Backend: 184 domain (+9: `RoleTests`) + 267 application (unchanged count; several fixture files updated to seed a `Role` instead of a `ProjectRole` enum value) + 7 architecture + 33 integration tests green. Frontend: `tsc -b`, `oxlint`, `vitest run` (85 tests) green. |
| 1.45 | 2026-08-21 | Board `WorkItemStatusDefinition` catalog, workflow editor, sprint insights panel, per-user board view settings, and sprint edit (§13.5 Agile UI, §13.5.1 Board settings) | Replaces the previously fixed `WorkItemStatus` enum (Backlog/Selected/InProgress/InReview/Done/Blocked) with a project-owned `WorkItemStatusDefinition` catalog (`Domain.Configuration`, mirroring the existing `WorkItemTypeDefinition` pattern) so a project can add, rename, recolor, recategorize, and reorder its own workflow statuses instead of being limited to the six built-ins — the change the §4.2 workflow-engine vision calls for, scoped here to statuses/categories only; conditions, validators, and post-functions on transitions remain future work (ADR-002, still "Partially" accepted). `WorkItem.StatusId`/`BoardColumn.StatusId` are now `Guid` foreign keys into the catalog instead of the enum; every project (new and existing) is seeded with the same six defaults the enum used to hard-code, via `WorkItemStatusDefinition.CreateSoftwareDefaults` at project creation and a one-time migration backfill for existing data (migration `AddWorkItemStatusCatalogAndBoardViewPreference`: seeds 114 status rows across existing projects, remaps `work_items.status`/`board_columns.status` to the new `status_id` FK by matching the old enum value's slug, then drops the old columns and check constraint — live-verified against the local dev database with zero orphaned references across 647 work items and 22 board columns). New `GET/POST/PATCH/DELETE /api/v1/projects/{id}/statuses` (`Application/Configuration/ManageWorkItemStatuses.cs`) back a new "Edit workflow" dialog reachable from the Board tab's `...` menu, matching the requested Jira-style "Add status" affordance; a status can't be deleted while it's the project's last one or still referenced by a work item or board column. `StatusCategory` (ToDo/InProgress/Done, already existed as an enum) is what the handful of places that need to reason generically about "is this done/in-progress" now key off — sprint completion's not-Done check, cycle-time/cumulative-flow folding, and the new insights panel — rather than a specific status id, so custom statuses participate in existing agile logic correctly; `WorkItemHistoryEntry` "Status" rows now record the status *key* (immutable even across a rename) instead of the enum's `ToString()`. Also new in this pass: `Sprint.Edit` (name/goal/dates, allowed for a future or active sprint unlike `Start`) behind `PATCH /api/v1/sprints/{id}`, surfaced as an "Edit sprint" dialog; a `SprintInsightsHandler` (`GET /sprints/{id}/insights`) computing live sprint progress (done/in-progress/not-started counts), an "items for attention" list (overdue/stuck-in-progress-5+-days/blocked-by-key/flagged, filterable by tab), and per-epic completion, surfaced as a right-hand panel reusing the same committed/completed/added/removed-points fold `SprintReportHandler` already had (extracted to shared `SprintReportPoints.Compute`); and a per-user, per-project `BoardViewPreference` (`GET/PATCH /projects/{id}/board-view-preference`, same zero-version-sentinel/`If-Match` shape as `ProjectSetting` but keyed by the caller's own user id rather than requiring project-admin permission) covering hide-done-items-after, column-size (fixed/flexible, also fixed a pre-existing bug where the Kanban grid was hardcoded to exactly six columns), and per-field visibility — scoped honestly to the three fields `WorkItemCard` actually renders (type/priority/assignee) rather than a longer decorative toggle list matching every field in the reference screenshots, since a toggle with no rendering effect would be a fake feature. Every tenant-scoped table gained forced-RLS policies in the same migration (`work_item_status_definitions`, `board_view_preferences`) — missing them was caught by a live 409-vs-migration-mismatch during smoke testing against the real API, not by the unit suite. Frontend: `WorkItemStatus` union type removed from `api/types.ts` in favor of `WorkItemStatusDefinition`/`statusId: string` everywhere (touched ~25 files: Board/Backlog/Timeline/Summary/Home views, work item detail/subtasks, rich-text ticket mentions, filters); every status label/color previously read from a static `statusMeta` table now resolves from the fetched per-project catalog. Backend: 196 domain (+21: `WorkItemStatusDefinitionTests`, `Sprint.Edit`) + 282 application (+38: `WorkItemStatusHandlerTests`, `SprintInsightsHandlerTests`, `BoardViewPreferenceHandlerTests`, `UpdateSprint*`, plus every existing status-touching fixture updated from the enum to a seeded catalog) + 7 architecture + 33 integration tests green; migration applied and live-verified against both the local dev database and the separate `orbit_test` integration database, plus a real `dotnet run` smoke test exercising create-status/board/sprint-insights/edit-sprint/delete-status end to end over HTTP. Frontend: `tsc -b`, `oxlint`, `vitest run` (85 tests) green; `npm run build` clean. Not attempted this pass, left for a future increment: guarded transitions (which status can move to which), and wiring the remaining reference-screenshot view-settings fields (Card cover, Confluence items, Development, Linked work items, Reporter, Resolved, Team, Created/Updated) into card rendering, since `WorkItemCard` doesn't render them yet. |
| 1.46 | 2026-08-21 | Review-fix pass on v1.45's status catalog: report-integrity guards, explicit default status, atomic board columns, diagram-style workflow editor (§13.5 Agile UI, §13.5.1 Board settings) | Fixes seven findings from a code review of v1.45. **Report integrity (the two High findings):** `IWorkItemStatusRepository.IsInUseAsync` now also checks `WorkItemHistoryEntry` rows keyed by the status's stable `Key` (not just current work-item/board-column references), because cumulative-flow/cycle-time reports resolve historical status transitions against the *current* catalog — recategorizing or deleting a status with history would silently reinterpret past reports. `UpdateWorkItemStatusHandler` now rejects a category change (but still allows rename/recolor/reorder) once `IsInUseAsync` is true; `DeleteWorkItemStatusHandler` rejects deletion on the same condition, replacing the old current-references-only check. **Explicit default status (the third High finding):** added `WorkItemStatusDefinition.IsDefault` (bool, a DB column with a partial unique index enforcing at most one default per project) plus `SetDefault`/`SetDefaultWorkItemStatusCommand`/`POST /projects/{id}/statuses/{id}/default`, so "the status new work items start in" is an explicit, independently-set flag instead of implicitly whichever status has the lowest `Order` — reordering a workflow no longer silently changes what new work items default to. `GetDefaultAsync` prefers the flagged row, falling back to lowest-`Order` only if none is flagged (covers rows from before the backfill). A default status can't be deleted (must reassign the default first). **Atomic board column (the Medium UX finding):** `CreateWorkItemStatusHandler` now also appends a matching column to the project's board (if one exists) in the same save, so a newly added status is immediately usable on the board instead of requiring a separate "Edit board" step to discover and save. **Sprint insights perf (the Medium finding):** `SprintInsightsHandler.BuildEpicProgressAsync` now takes the already-fetched status dictionary instead of re-querying it, and replaced its O(epics × sprint-items) `Where(item => item.ParentId == epicId)` scan with a single `ToLookup(item => item.ParentId!.Value)` built once up front. **Frontend fixes (the two Low findings):** `WorkflowEditorDialog.tsx`'s Text-tab name input now remounts on `${status.id}-${status.version}` instead of relying on a `defaultValue` that goes stale after a React Query refetch of a renamed status; `App.tsx`'s `activeSprint` lookup now matches `state === 'Active' or 'Reopened'` (previously only `'Active'`) so Sprint insights and Edit sprint don't disappear from the Board `...` menu after a sprint is reopened, matching `Sprint.StartClosing`'s own Active-or-Reopened acceptance. **Screenshot-parity redesign** (separate from the review, requested alongside it — the v1.45 View Settings/Edit Workflow/Edit Sprint UIs were functionally adequate but didn't visually match the supplied Jira reference screenshots): `BoardViewSettingsPanel.tsx` rewritten with a search box, custom toggle-switches, and the reference's full ~21-field "Show fields" list — each toggle either wired to a real `WorkItemCard` render (assignee, created, dueDate, flagged, labels, parent, priority, startDate, status, storyPointEstimate, subtaskSummary, summary, updated, workItemKey) or shown disabled with a "Not available yet" tooltip for fields Orbit has no backing data for (Card cover, Confluence items, Development, Linked work items, Reporter, Resolved, Team) — closing the "toggle list only covered 3 fields" gap v1.45 left open, without faking a toggle that does nothing. `WorkItemCard.tsx` gained rendering for all the newly-wired fields (parent chip, subtask summary, flagged icon, date chips, label chips, created/updated relative-time text). `SprintEditDialog.tsx` rewritten to match the reference's asterisked-required-field layout, calendar-icon date inputs, and a disabled (backend doesn't support it) "auto-complete on end date" checkbox. `WorkflowEditorDialog.tsx` rewritten with a Diagram/Text tab toggle: the new default Diagram tab renders a horizontal node graph (START → Create → each status, connected by "Any"-labeled arrows, since transitions are genuinely unrestricted here — no guarded-transition engine exists, so the reference screenshot's Add Transition/Add Rule/Add agent toolbar buttons were deliberately not built, as they'd be fake features with no backend) with a click-to-select edit panel (rename/recategorize/recolor/reorder/set-default/delete), and a redesigned "Add status" modal matching the reference's small Name+Category dialog; the Text tab keeps the original table-row editor as a fallback, now also exposing set-default. Backend: 196 domain + 287 application (+5: `UpdateWorkItemStatus_AllowsPresentationChanges_WhenInUse`, `UpdateWorkItemStatus_RejectsRecategorization_WhenInUse`, `SetDefaultWorkItemStatus_MovesDefaultFlagBetweenStatuses`, `DeleteWorkItemStatus_RejectsDefaultStatus`, `CreateWorkItemStatus_AppendsBoardColumn_WhenBoardExists`) + 7 architecture + 33 integration tests green (523 total); migration `AddWorkItemStatusIsDefault` applied and live-verified against both the local dev database (19 projects, exactly one default status each) and `orbit_test`. Frontend: `tsc -b`, `oxlint`, `vitest run` (85 tests) green; `npm run build` clean. |
| 1.47 | 2026-08-22 | Sprint-close rollover redesign and board/backlog-access notifications (§13.5 Agile UI, §13.5 Notifications) | Replaces the "pick an existing Future sprint" rollover dropdown with a yes/no decision: `CompleteSprintCommand.RolloverTargetSprintId: Guid?` is now `CreateRolloverSprint: bool`. Choosing yes auto-creates the next sprint (`Sprint.NextRolloverName`, a pure domain helper: increments the closed sprint's trailing number preserving zero-padding, e.g. "Sprint 05" → "Sprint 06", falling back to a date-based name when there's no trailing number; new sprint's dates are the day after the closed sprint's end date, spanning the same duration; created `Future`, not auto-started) and moves every not-Done item into it via the existing membership-swap path. Choosing no reorders those items to the very top of the backlog: a new `IWorkItemRepository.GetMinBacklogRankAsync` (excludes items with a current membership in any non-Closed sprint, mirroring `BacklogView.tsx`'s own backlog definition) returns the lowest existing rank, and moved items are assigned ranks below it in `RankGap = 1024m` steps (same constant `ReorderWorkItemHandler` uses), preserving their relative order. The idempotent-retry path (`SprintCompletionOperation.RolloverTargetSprintId`) now holds the id of the sprint the handler itself created rather than a caller-supplied id — the new sprint and the operation row are written in the same `SaveChangesAsync`, so an interrupted close rolls both back together and a resumed request (`operation is not null`) reuses the stored id instead of creating a second sprint. Sprint-completed notifications changed from the sprint's work-item owner fields (Assignee/Developer/ProductOwner) to every user holding `ProjectPermission.View` on the project — everyone with board/backlog access, not just item owners — via a new `IProjectAccessRepository`/`ProjectAccessRepository` (`src/Orbit.Infrastructure/Persistence/ProjectAccessRepository.cs`) whose query is the enumeration-shape inverse of the existing `ProjectAccessQuery.PermittedProjects` (same tenant-wide Owner/Administrator shortcut, explicit `ProjectRoleAssignment`, and `ProjectGroupRoleAssignment`-via-`GroupMembership` paths, but "which users see this project" instead of "which projects does this user see"). `SprintNotifications` was split into a shared recipient-agnostic send helper plus two callers: the existing owner-based `NotifyAsync` (still used by `StartSprintHandler`, unchanged) and a new `NotifyProjectViewersAsync` (used only by `CompleteSprintHandler`). Frontend: `CompleteSprintDialog.tsx`'s dropdown and `BacklogView.tsx`'s separate inline rollover `SearchableSelect` were both replaced by a shared `RolloverChoice.tsx` yes/no control, which previews the auto-created sprint's name via a `nextRolloverName` TypeScript port of the domain naming rule (`web/src/lib/sprintNaming.ts`) so a PM sees the real name before confirming; `App.tsx`'s `completeSprintMutation` now invalidates the sprints query on a "yes" close, since the response only carries the closed sprint and the new one isn't in cache otherwise. Backend: 203 domain (+6: `Sprint.NextRolloverName` cases) + 295 application (+8: top-of-backlog rank assertion, View-permission-recipient notification test, auto-create-and-rollover test, resume-does-not-duplicate-sprint test, replacing the deleted existing-Future-sprint-target tests) + 7 architecture + 45 integration (+2: `ProjectAccessRepositoryTests`/`WorkItemBacklogRankTests` against real Postgres, seeding tenant context directly via `TenantContext.SetTenant` since these exercise a repository query in isolation rather than a full HTTP request) tests green — 550 total. Frontend: `tsc -b`, `oxlint`, `vitest run` (97 tests, +4 `sprintNaming.test.ts`) green; `npm run build` clean. Not attempted this pass: a live-browser click-through of the redesigned dialogs (verify visually before shipping), and multi-instance rollover targeting (still exactly one auto-created sprint per close). |
| 1.48 | 2026-08-23 | Backlog insights/view-settings/inline preview, real sprint-insights data, and back-filled v1.47 polish (§13.5.6) | Two parts, detailed in §13.5.6. **New this pass:** `BacklogInsightsPanel.tsx` (planning-sprint commitment points, top-5 work-type breakdown) and `BacklogViewSettingsPanel.tsx` (row density, empty-sprint visibility, per-column field toggles) open from new `BacklogView.tsx` toolbar buttons; backlog row selection now opens an inline, pointer-draggable split-view `WorkItemDetailView` (width persisted to `localStorage`) instead of only the existing drawer/modal, both of which shrank from near-full-viewport to a smaller floating dialog (`App.css`) so the same detail component reads correctly at every width it's now embedded at. `SprintInsightsPanel.tsx`'s burndown chart and Added/Removed/Net-change scope tiles, hardcoded since v1.45, now render from `GET /sprints/{id}/report` and two new `SprintInsightsDto` fields (`AddedAfterStartCount`/`RemovedAfterStartCount`, `src/Orbit.Application/Boards/SprintInsights.cs`, computed by the same fold already used internally). All of the above is frontend-only plus the two additive DTO fields — no migration, no new endpoint. Not yet done: persisting backlog view settings and an insights velocity recommendation (both UI-only and session-scoped today). **Back-filled from the same `b603890` commit that shipped v1.47's rollover redesign, undocumented until now:** `CustomFiltersPanel.tsx` gained a "Preview" banner, five example WQL query-preset chips, and an inline syntax-reference panel — still explicitly non-persisting (no WQL execution engine exists, per §13.5 Agile query row), so this is sample/reference content for a form that doesn't yet submit anywhere; `SearchableSelect.tsx` gained a clear-selection control and debounced (`requestAnimationFrame`) reposition-on-scroll instead of firing on every scroll event; `ErrorScreen`/`ErrorBoundary.tsx` now surfaces `ApiError.correlationId` on the fallback screen so a user reporting a crash can hand support the same id already tagged on the captured Sentry event. Test evidence: `dotnet build Orbit.slnx` clean, no new backend tests needed (additive DTO fields only); frontend `tsc -b && vite build` clean, `vitest run` 109 tests green (30 files, +1 new `BacklogView.test.tsx` covering open/close of the inline split panel), `oxlint` clean apart from two pre-existing `exhaustive-deps` warnings in `BacklogView.tsx` unrelated to this pass. |
| 1.49 | 2026-08-24 | Backlog view settings persistence | Closes the "Not yet done: persisting backlog view settings" gap flagged in v1.48/§13.5.6. New `BacklogViewPreference` aggregate (`Orbit.Domain.Settings`, migration `AddBacklogViewPreference`, forced RLS) mirrors `BoardViewPreference` exactly: a per-user, per-project row (`show_empty_sprints`, `density`, `hidden_fields`) with the same zero-version-sentinel GET / `If-Match` PATCH shape (`GET`/`PATCH /api/v1/projects/{id}/backlog-view-preference`, `ManageBacklogViewPreference.cs`, `BacklogViewPreferenceEndpoints.cs`). `BacklogViewSettingsPanel.tsx` was rewritten from prop-drilled local `useState` (settings/onChange from `BacklogView.tsx`) to self-fetching/self-mutating via its own `projectId` prop, matching `BoardViewSettingsPanel.tsx`'s existing shape; `BacklogView.tsx` now reads the same `['backlog-view-preference', projectId]` query (deduped by React Query, no extra request) to gate epic/due-date/status/estimate column rendering and row density instead of local component state, so the settings now survive reload and follow the user across devices. Field visibility reuses `BoardViewPreference.HiddenFields`'s inverted convention (a field is hidden only once explicitly opted out) rather than the frontend's prior positive `showX` flags, for consistency with the one other view-preference aggregate in the codebase. Test evidence: `dotnet test Orbit.slnx` 299 application + 203 domain + 7 architecture tests green (3 new `BacklogViewPreferenceHandlerTests`, mirroring `BoardViewPreferenceHandlerTests`); migration applied cleanly against the local Postgres stack; frontend `tsc -b && vite build` clean, `vitest run` 113 tests green, `oxlint` clean apart from the same two pre-existing `exhaustive-deps` warnings noted in v1.48. Not attempted: the insights panel's velocity-average recommendation (a separate, unrelated gap noted in the same v1.48 entry). |
...
**Partially built.** Orbit expands the Workspace Identity model to support explicit **Teams** and strict **Role-Based Access Control (RBAC)** to match complex enterprise governance models. The role matrix and invitation lifecycle below are deepened from the original bullet points using Jira's own project-role + group model as the reference shape (not queried live against a Jira instance this pass). **Project roles are no longer a fixed row in this table** — as of v1.44 (§4.3, §13.5) `Role`/`RolePermission` is a tenant-scoped, admin-editable schema, so "Member"/"Viewer" below are just the two non-administrator system roles every tenant is seeded with, not fixed capabilities; a tenant admin can rename their permission sets, remove permissions from them, or add further custom roles. Site super admin, workspace owner/admin, and team admin remain proposed/enum-based, tracked in §13.5's Access control row as the still-open "tenant/workspace-level custom roles" increment.
...
| Workspace admin | Workspace | Manage members/teams/roles (`ITenantAuthorization.CanManageRoles`, implemented v1.44), workspace settings, billing, integrations | Appoint site super admins |
| Team admin *(proposed)* | Team | Add/remove team members, rename team | Change project roles outside the team's granted projects |
| Administrator *(system role, editable)* | Project (role-granted) | Configure project settings, assign project roles; whatever `ProjectPermission`s the role currently grants (all four by default) | Manage workspace-level membership |
| Member *(system role, editable)* | Project (role-granted) | Create/edit/transition work items per the role's granted `ProjectPermission`s (View/CreateWorkItem/TransitionWorkItem by default) | Whatever permissions the role doesn't grant |
| Viewer *(system role, editable)* | Project (role-granted) | Read work items, boards, reports (View by default) | Whatever permissions the role doesn't grant |
| *(custom role)* | Project (role-granted) | Exactly the `ProjectPermission`s an admin assigns it | Whatever permissions the role doesn't grant |
...
- **RBAC Over ABAC:** Project access is evaluated via project-role assignment (direct or directory-group, implemented, and — as of v1.44 — against admin-defined `Role`/`RolePermission` grants rather than a fixed enum) with dynamic issue-security predicates (§4.3) remaining proposed.
