...
| Data | EF Core/Npgsql migrations, PostgreSQL choice/role constraints, tenant-composite foreign keys, indexes, forced RLS for work and access tables (including `sprints`/`sprint_memberships`); a global, unfiltered `outbox_email_messages` table for transactional email | Field definitions/options, typed projections, workflow graph, sprint scope facts, audit partitions |
...
2. ~~Implement workspace creation/selection, site and workspace admin management, teams, team membership, invitation delivery/acceptance, and audit.~~ **Done for workspace admin lifecycle, teams, groups, and project-group roles:** owner changes are serialized per tenant, and production now rejects database roles that bypass RLS. Remaining: workspace creation/selection UX, invitation delivery/acceptance (can now reuse the outbox from step 7), and audit.
...
7. ~~Add transactional outbox/jobs and make `Orbit.Worker` process notifications, projections, invitations, imports, and automation under tenant-scoped leases.~~ **Done for the email outbox (v1.15):** `outbox_email_messages` is a global (unfiltered, no RLS) table written in the same `SaveChangesAsync` as the domain change that triggers it — e.g. a `PasswordResetToken` — so the write is atomic with the outbox row per ADR-014. `Orbit.Worker` now calls `AddInfrastructure` and runs `OutboxDispatchWorker`, a `PeriodicTimer`-driven `BackgroundService` that claims a bounded batch with `FOR UPDATE SKIP LOCKED` inside one transaction (row-lock based rather than a time-based lease — simpler and sufficient at the current single-worker scale) and delivers via SMTP (MailKit; Mailpit for local dev, see `deploy/podman/compose.yaml`). Failed sends are retried up to 5 attempts and then left unpublished (bounded retry, no dead-letter table yet). Scope is deliberately email-only, not the fully generic `outbox_event`/`EventType` design this section originally sketched — that generalization is deferred until a second, non-email consumer actually needs it. Remaining: projections, invitations, imports, and automation as outbox consumers; tenant-scoped leases (today's claim is global, matching the global table); a dead-letter/retry operator UI (`§9.2` S1.4.4).
