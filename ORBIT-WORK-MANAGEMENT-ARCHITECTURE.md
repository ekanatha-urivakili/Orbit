...
|---|---|---|
| Backend | .NET 10 headless Minimal API; Clean Architecture; CQRS/MediatR; FluentValidation; RFC 7807 errors | API version policy, idempotency records, rate limiting, generated SDK contract tests |
| Tenancy | Global user account and workspace persistence; one-time advisory-lock bootstrap; local email/password login with rotating refresh-session families, theft-on-replay detection, logout, and session list/revoke (`POST /auth/login`\|`/refresh`\|`/logout`, `GET/DELETE /me/sessions`); locally-signed JWT bearer tokens carry `tenant_id`, so the tenant claim now resolves without the dev header; development-only `X-Tenant-Id` remains for local convenience; user-linked or OIDC subject membership; global EF tenant filters; transaction-local PostgreSQL RLS, including a narrow `tenant_memberships_self_lookup` policy so login/refresh can resolve a user's own memberships before a workspace is selected | Password reset (needs the email/outbox worker), multi-workspace switcher UX, connection-reuse isolation suite, **and provisioning the `NOSUPERUSER NOBYPASSRLS` runtime role that ADR-006/NFR-07 require — every current environment (Podman, CI, Railway docs) still connects as the schema-owning role, so RLS is not yet enforced anywhere it runs** |
| Access control | Owner/admin/member tenant roles; administrator/member/viewer project roles; tenant-composite role assignments; permission predicates inside project/work-item queries; existence-hiding reads; workspace admin lifecycle — promote/demote a member's tenant role and deactivate a membership, both guarded so a workspace can never lose its last active owner (`PUT`/`DELETE /api/v1/memberships/{id}`\|`/role`) | Directory groups, permission schemes and dynamic grants, authorization epoch/cache, issue security, administration UI and audit |
| Workspace directory | Bootstrap-created first workspace and owner membership; tenant memberships and project roles; workspace-scoped team directory and team membership (`/api/v1/teams`, `/api/v1/teams/{id}/members`) with a settings UI panel | Workspace selection, invitation and acceptance lifecycle (email delivery blocked on the outbox worker in step 7), verified-email account linking |
| Work items | Create/list; stable Initiative, Epic, Task, Story, Bug, Spike, Test, Feature and Request choices; typed creation fields; portfolio hierarchy; self-referencing dependency links; ownership ids; labels/countries arrays; attachment-name metadata; `If-Match` status transition; versioned full-field update (`PATCH /api/v1/work-items/{id}`, summary/description/priority/hierarchy/ownership/labels, `WorkItemType` immutable post-creation) | Configurable field/screen registry, multi-link relation table, quarantined object upload, audit history |
| Agile UI | Persisted project-owned board aggregate with Kanban/Scrum creation and rename UI (`GET`/`PATCH /api/v1/projects/{id}/board`, `If-Match` versioned); configurable status-to-column mapping with per-column ordering and WIP limits in Warn or Block mode, editable from the board settings form and enforced when moving a work item into a Block column; fractional-rank drag reorder within a column (`PATCH /work-items/{id}/rank`) with atomic card moves; persisted `Sprint` aggregate with Future/Active/Closed lifecycle (`POST /api/v1/projects/{id}/sprints`, `GET` list, `POST /api/v1/sprints/{id}/start`\|`/complete`, `If-Match` versioned) enforcing a single active sprint per project; temporal `sprint_memberships` recording add/remove events rather than a mutable work-item field (ADR-013); `PUT`/`DELETE /api/v1/work-items/{id}/sprint` to move items between the backlog and an open sprint; Backlog tab renders real sprint sections instead of a client-side mock; home surface; clickable Orbit logo; distinct settings/profile menus; type-aware create dialog; project onboarding; accessible status move control | Parallel active sprints, durable sprint-completion workflow with a rollover target picker (today's complete is a single synchronous transaction that returns incomplete items straight to the backlog), sprint reopening, `sprint_scope_fact` audit rows for burndown/velocity reports, swimlanes, virtualization, cross-column drag (status changes are select-driven, not drag-and-drop) |
...
| Data | EF Core/Npgsql migrations, PostgreSQL choice/role constraints, tenant-composite foreign keys, indexes, forced RLS for work and access tables (including `sprints`/`sprint_memberships`) | Field definitions/options, typed projections, workflow graph, sprint scope facts, outbox/jobs, audit partitions |
| Delivery | Podman PostgreSQL 18 and Valkey 9.1, separate OCI files, Railway service configs, SHA-pinned GitHub Actions, pinned local `dotnet-ef` tool manifest (`.config/dotnet-tools.json`) restored before every migration step so CI/Railway do not depend on an implicitly pre-installed global tool | GHCR digest promotion, SBOM/provenance/signing, preview environments, migration compatibility gate, rollback automation, provisioning the restricted runtime database role noted under Tenancy above |
...
| Notifications | Event/channel/digest/quiet-hours/self-notify controls | Versioned preference API and UI implemented | Consume preferences in notification delivery; add preview/test delivery |
...
| Workspace settings | General details, members, teams, roles, invitations, security, integrations, billing | General defaults and member project-creation policy implemented with RLS and admin enforcement; member role change/removal and a team directory/membership module now implemented (v1.10) | Add invitations, security, integration, and billing modules, audit and authorization-epoch invalidation |
| Project settings | Details, people, item types, workflows, fields, permissions, automation, integrations | Default item type/priority and feature toggles implemented with project-admin enforcement | Add draft/publish configuration, people, schemes, impact preview and audit |
...
1. ~~Finish local identity after the implemented global account, Argon2id credential, atomic super-admin bootstrap, first workspace, and user-linked owner membership: add login/reset/rotating sessions~~ **Done for login/rotating sessions (v1.9):** `POST /api/v1/auth/login|refresh|logout` and `GET/DELETE /api/v1/me/sessions` are implemented — local email/password login issues a locally-signed JWT access token plus an opaque rotating refresh token; refresh tokens form a family, reuse of an already-rotated or revoked token revokes the whole family (theft response), and login/refresh timing is enumeration-resistant (constant-cost Argon2 verify against a dummy hash for unknown accounts). Remaining: password reset (blocked on the email/outbox worker in step 7) and migrating existing external identities through explicit account linking.
2. ~~Implement workspace creation/selection, site and workspace admin management, teams, team membership, invitation delivery/acceptance, and audit.~~ **Done for workspace admin lifecycle and teams (v1.10):** `PUT /api/v1/memberships/{id}/role` and `DELETE /api/v1/memberships/{id}` let an owner (any role) or administrator (member-role only) promote, demote, or remove a workspace member, guarded so the workspace can never lose its last active owner; `/api/v1/teams` and `/api/v1/teams/{id}/members` add a workspace-scoped team directory and membership, with a settings UI panel. v1.10 also closed a login/refresh correctness gap found while implementing this: `ListActiveMembershipsByUserAsync` queried `tenant_memberships` (which carries `FORCE ROW LEVEL SECURITY`) with no `app.tenant_id` set, since login runs before a workspace is selected; a new `tenant_memberships_self_lookup` SELECT policy, gated on a separate `app.principal_user_id` GUC the repository now sets, permits that one cross-tenant self-lookup without weakening tenant isolation on writes or on any other read. This was verified directly against Postgres with a temporary `NOSUPERUSER NOBYPASSRLS` probe role, which also confirmed the finding under "Tenancy" above: no shipped environment runs as such a role today, so RLS was not actually being exercised before this fix and is not enforced by any deployment yet. Remaining: workspace creation/selection UX, invitation delivery/acceptance (still blocked on the email/outbox worker in step 7 — a workspace admin adds members directly via `POST /api/v1/memberships` in the meantime), and audit.
3. ~~Replace the implemented stable choice baseline with a configurable item-type registry; implement project-owned Kanban/Scrum board definitions and creation UI.~~ **Done for the board aggregate and creation UI (v1.11):** `GET`/`PATCH /api/v1/projects/{id}/board` persist a versioned `Board` (name, `BoardType`) with the same zero-version-sentinel/`If-Match` pattern as workspace and project settings; the Board tab's inline form creates or renames it (project-Administer permission required, `NotFoundException` on the existence-hiding path when the caller lacks it). The configurable item-type registry itself (replacing the stable `WorkItemType`/`WorkItemStatus` enums with tenant-defined types and schemes) remains outstanding — this step only covers the board aggregate half.
...
5. Finish identity hardening: browser OIDC PKCE/account linking, directory groups, authorization epoch/cache, service-account conformance, and connection-reuse isolation tests.
6. Add versioned custom field definitions and choice options, screens, typed query projections, and WQL.
7. Add transactional outbox/jobs and make `Orbit.Worker` process notifications, projections, invitations, imports, and automation under tenant-scoped leases.
...
| 1.9 | 2026-08-12 | Local login and session lifecycle | Implemented FR-32/NFR-17 local email/password login (`POST /api/v1/auth/login`), rotating refresh sessions with reuse/theft detection (`/auth/refresh`), logout (`/auth/logout`), and session review/revocation (`GET/DELETE /api/v1/me/sessions`); added locally-signed JWT bearer validation as the self-hosted identity path (ADR-019, CON-09) so authenticated requests resolve `tenant_id` from the token instead of requiring the dev-only `X-Tenant-Id` header; password reset remains blocked on the email/outbox worker (§13.5 step 7) |
