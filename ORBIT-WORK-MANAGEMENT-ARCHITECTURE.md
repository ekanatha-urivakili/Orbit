...
| Tenancy | Global user account and workspace persistence; one-time advisory-lock bootstrap; local email/password login with concurrency-safe rotating refresh-session families; locally-signed and external OIDC bearer validation; proof-validated external identity linking; complete EF tenant filters; transaction-local PostgreSQL RLS; production startup rejection of `SUPERUSER`/`BYPASSRLS` runtime roles; separate migration-owner connection | Password reset (needs the email/outbox worker), multi-workspace switcher UX, connection-reuse isolation suite |
...
2. ~~Implement workspace creation/selection, site and workspace admin management, teams, team membership, invitation delivery/acceptance, and audit.~~ **Done for workspace admin lifecycle, teams, groups, and project-group roles:** owner changes are serialized per tenant, and production now rejects database roles that bypass RLS. Remaining: workspace creation/selection UX, invitation delivery/acceptance, and audit.
...
5. Finish identity hardening: service-account conformance, password recovery, invitation proofs, and connection-reuse isolation tests. Browser OIDC PKCE/account linking now validates the IdP-issued identity token server-side; directory groups and authorization epoch/cache are implemented.
